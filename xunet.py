...
def nearest_neighbor_upsample(h):
    ...
    h = torch.nn.functional.interpolate(
        h, scale_factor=(1, 2, 2), mode='nearest')
...
def posenc_ddpm(timesteps, emb_ch: int, max_time=1000.):
    ...
    emb = torch.exp(torch.arange(half_dim) * -
                    emb).to(device=timesteps.get_device())
...
def posenc_nerf(x, min_deg=0, max_deg=15):
    ...
    if min_deg == max_deg:
        return x
    scales = torch.tensor(
        [2**i for i in range(min_deg, max_deg)]).float().to(x.get_device())
...
class FiLM(torch.nn.Module):
    ...
    def forward(self, h, emb):
        ...
        emb = self.dense(torch.nn.functional.silu(
            emb.transpose(-1, -3))).transpose(-1, -3)
...
class ResnetBlock(torch.nn.Module):
    ...
    def forward(self, h_in, emb):
        ...
        h = self.film(h, emb)
        h = self.dropout(h)
        h = self.conv2(h.view(B*F, self.features, H, W)
                       ).view(B, F, self.features, H, W)
...
class AttnLayer(torch.nn.Module):
    ...
    def __init__(self, attn_heads=4, in_channels=32):
        ...
        self.attn = torch.nn.MultiheadAttention(
            in_channels, attn_heads, batch_first=True)
...
class AttnBlock(torch.nn.Module):
    ...
    def __init__(self, attn_type, attn_heads=4, in_channels=32):
        ...
        self.attn_layer = AttnLayer(
            attn_heads=attn_heads, in_channels=in_channels)
...
class ConditioningProcessor(torch.nn.Module):
    ...
    def __init__(self, emb_ch, H, W,
        ...
        self.use_ref_depth_emb = use_ref_depth_emb
        ...
        if use_depth_emb:
            ...
            torch.nn.init.normal_(self.depth_emb, std=(1 / np.sqrt(D)))

        if use_ref_depth_emb:

            self.first_emb = torch.nn.Parameter(
                torch.zeros(1, 1, D, 1, 1), requires_grad=True)
            torch.nn.init.normal_(self.first_emb, std=(1 / np.sqrt(D)))

            self.other_emb = torch.nn.Parameter(
                torch.zeros(1, 1, D, 1, 1), requires_grad=True)
    ...
    def forward(self, batch, cond_mask):
        ...
        depth_emb = torch.where(cond_mask, depth_emb, torch.zeros_like(
            depth_emb))  # [B, F, H, W, 144]
        depth_emb = rearrange(depth_emb, "b f h w c -> b f c h w")

        if self.use_depth_emb:
            depth_emb += self.depth_emb[None, None]
        if self.use_ref_depth_emb:
            depth_emb = torch.concat(
                [self.first_emb, self.other_emb], axis=1) + depth_emb

        depth_embs = []
        for i_level in range(self.num_resolutions):
            B, F = depth_emb.shape[:2]
            depth_embs.append(
                rearrange(
                    self.convs[i_level](
                        rearrange(depth_emb, 'b f c h w -> (b f) c h w')
                    ),
                    '(b f) c h w -> b f c h w', b=B, f=F
                )
            )

        return logsnr_emb, depth_embs
...
class XUNet(torch.nn.Module):
    ...
    # actually, level of depth, from 0 th N
    attn_resolutions: tuple[int] = (2, 3, 4)
    attn_heads: int = 4
    dropout: float = 0.1
    use_depth_emb: bool = True
    use_ref_depth_emb: bool = True

    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)
        super().__init__()

        assert self.H % (2**(len(self.ch_mult)-1)
                         ) == 0, f"Size of the image must me multiple of {2**(len(self.ch_mult)-1)}"
        assert self.W % (2**(len(self.ch_mult)-1)
                         ) == 0, f"Size of the image must me multiple of {2**(len(self.ch_mult)-1)}"
        ...
        self.conditioningprocessor = ConditioningProcessor(
            ...
            use_depth_emb=self.use_depth_emb,
            use_ref_depth_emb=self.use_ref_depth_emb,
            H=self.H,
            W=self.W
        )

        self.conv = torch.nn.Conv2d(
            self.input_ch, self.feature_ch, kernel_size=3, stride=1, padding='same')

        # channel size
        self.dim_in = [self.feature_ch] + \
            (self.feature_ch * np.array(self.ch_mult)[:-1]).tolist()
        self.dim_out = (self.feature_ch * np.array(self.ch_mult)).tolist()
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks + 1):
                ...
                if i_block == 0:
                    ...
                    prev_h_channels = self.dim_out[i_level+1] if (
                        i_level+1 < len(self.dim_out)) else self.dim_out[i_level]
        ...
        self.lastgn = GroupNorm(num_channels=self.feature_ch)
        self.lastconv = torch.nn.Conv2d(
            in_channels=self.feature_ch, out_channels=self.output_ch, kernel_size=3, stride=1, padding='same')
    ...
    def forward(self, batch, *, cond_mask):
        ...
        logsnr_emb, depth_embs = self.conditioningprocessor(batch, cond_mask)
        ...
        for i_level in range(self.num_resolutions):
            ...
            emb = logsnr_emb[..., None, None] + depth_embs[i_level]
        ...
        # middle, 1x block
        emb = logsnr_emb[..., None, None] + depth_embs[-1]
...
if __name__ == "__main__":
    ...
    a = torch.nn.DataParallel(
        XUNet(H=h, W=w, input_ch=2, output_ch=2, feature_ch=128)).cuda()

    batch = {
        'x': torch.zeros(b, 2, h, w).cuda(),
        'z': torch.zeros(b, 1, h, w).repeat(1, 2, 1, 1).cuda(),
        'logsnr': torch.tensor([10] * (2 * b)).reshape(b, 2),
        'depth': torch.randint(0, 256, (b, 2)).float().cuda(),
    }
